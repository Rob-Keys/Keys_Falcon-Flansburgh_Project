...
```
```{r}
# Resilience (BRS) as a function of athletic identity (AIMS) 

ai <- originalData %>% select(`AIMS_ TOTAL`, RES_TOTAL)
ai <- ai %>% filter(`AIMS_ TOTAL` <= 100, RES_TOTAL <= 100)


athleticIdentity <- ai %>% select('AIMS_ TOTAL')
athleticIdentity = athleticIdentity[athleticIdentity < 100]

resilience <- ai %>% select('RES_TOTAL')
resilience <- resilience[resilience < 100]

plot(athleticIdentity,resilience,main="Scatter")
fit <- lm(athleticIdentity ~ resilience, data = ai)
#plot(fit)
summary(fit)
```
