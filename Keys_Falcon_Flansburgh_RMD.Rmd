...
```
```{r}
# MCH-SF (a way to measure wellbeing) as a function of athletic identity (AIMS)
ai <- originalData %>% select(`AIMS_ TOTAL`, `MHC-SF OVERALL`)
ai <- ai %>% filter(`AIMS_ TOTAL` <= 100, `MHC-SF OVERALL` <= 100)


athleticIdentity <- ai %>% select('AIMS_ TOTAL')
athleticIdentity = athleticIdentity[athleticIdentity==athleticIdentity] # i know this is weird, idk why but this line converts it from a list to a plottable data type

happiness <- ai %>% select('MHC-SF OVERALL')
happiness <- happiness[happiness==happiness]

plot(athleticIdentity,happiness,main="Scatter")
fit <- lm(athleticIdentity ~ happiness, data = ai)
abline(fit,col="red")
plot(fit)
summary(fit)
```
